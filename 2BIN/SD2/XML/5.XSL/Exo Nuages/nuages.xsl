...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="nuage">
		<h2><xsl:value-of select="nom"/></h2> <!-- affiche l'attribut -->
		...
		<xsl:apply-templates select="altitude">
			<xsl:with-param name="nomNuage" select="nom"/> <!-- passe le nom du nuage en param -->
		</xsl:apply-templates>
	</xsl:template>

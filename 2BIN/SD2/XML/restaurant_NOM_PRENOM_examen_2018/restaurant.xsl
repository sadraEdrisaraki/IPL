...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="client">
		...
		<xsl:if test="count(//commande[@client=$idClient]) = 0"> <!-- Compte le nombre de commande pour le client -->
			<p> Pas de commande </p>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="commande">
		...
		<xsl:if test="@client=$idClient">	<!-- affiche que les commandes adéquate -->
			<p> 

...
public class PriorityQueue<Key> {
	...
	//les valeurs des nœuds de l’arbre dans l’ordre dfs
	public void dfs() {
		
		Deque<Integer> stack = new ArrayDeque<Integer>(); // contient les positions dans le tableau
		
		Integer current = 1; // l'indice du sommet à visité
		stack.push(current);
		
		while(!stack.isEmpty()) {
			current = stack.pop(); // on pop car on ne reviendra plus sur le sommet comme on a parcouru ses 2 fils. comme on est dans un arbre on peut faire ça

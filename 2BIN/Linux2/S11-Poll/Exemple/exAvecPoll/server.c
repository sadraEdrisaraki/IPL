...
int main(int argc, char **argv)
{
	...
	struct pollfd fds[1024]; // tableau des fd à surveiller
	...
	// on va surveiller le fd du socket pour les futurs connexion, comme ça la fonction accepte() n'est 
	// plus bloquante
	fds[nbSockfd].fd = sockfd; 
	fds[nbSockfd].events = POLLIN; // plus précisement la lecture du fd
	nbSockfd++;
	fds_invalid[nbSockfd] = false;

	while (1)
	{
		ret = poll(fds, nbSockfd, 0); // appel systeme Poll, pas de timeout
		checkNeg(ret, "server poll error");

		// trt accept : la fonction accept() qui est pollin : ça veut dire qu'il y a quelque chose à lire
		if (fds[0].revents & POLLIN & !fds_invalid[0])
		{
			...
			fds[nbSockfd].fd = newsockfd; // ajout du fd du client, pour le surveiller.
			fds[nbSockfd].events = POLLIN;
			nbSockfd++;
			fds_invalid[nbSockfd] = false;
		}

		// trt messages clients : parcours tous les fd des clients
		for (i = 1; i < nbSockfd; i++)
		{
			if (fds[i].revents & POLLIN & !fds_invalid[i])
			{
				...
				printf("%s\n", buffer);
				ret = write(fds[i].fd, buffer, sizeof(msg));
